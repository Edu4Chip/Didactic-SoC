...
void uart_init(){
  ...
  *( volatile uint32_t* )(0x01040028) = (temp | 3u);
  ...
  IIR_FCR = 1u;
  LCR = (1u<<7 | 3u);
  //RBR_THR_DLL = 27u;//divisor to define baudrate: ~230400 = (frequency, 100MHz)/(16*RBR_THR_DLL) // ASIC
  RBR_THR_DLL = 27u;//divisor to define baudrate: ~57600 = (frequency, 25MHz)/(16*RBR_THR_DLL) // FPGA
  LCR = 3u;
